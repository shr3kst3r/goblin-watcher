...
- **`HeadlessWindower`** (opt-in via `windowing = "headless"`): no terminal at all. `Popen`s the agent's print mode with `start_new_session=True`, stdin on `/dev/null`, stdout+stderr appended to `<project>/.goblin/logs/<task>-<session>.log`, and the child pid in a sibling `.pid` sidecar. Returns 0 as soon as the spawn succeeds. The sidecar is load-bearing beyond "something to `kill`": it is the crispest signal that an agent is still inside a worktree, which is what stops a prune from deleting one out from under a run that just merged its own PR (`commands/task.busy_reasons`, #56). `live_run_pids` reaps a sidecar whose process is gone, so a crashed run can't leave a task unprunable. This is what makes `gw` runnable from cron or a queue — and composable with itself. See ADR 0007.
...
So each agent declares the fact up front, as a `TranscriptCapability` (`agents/base.py`) class attribute: `parseable`, plus a short `reason` naming the obstacle when it isn't. Parseable agents use the `PARSEABLE_TRANSCRIPTS` singleton; today that's claude, codex, and antigravity, while gemini and managed each carry a reason.

`gw doctor` renders one advisory row per non-parseable agent (`<name> transcripts`), splicing the reason into a fixed sentence about the consequence — so the wording of *what you lose* lives in `commands/doctor.py` and each agent only owns *why*. The rows are generated by walking `AGENT_NAMES`, so a new agent gets one for free. They report `ok`: a stubbed reader is a property of the agent's session store, not a broken install, and doctor exits non-zero on any failed check.

The declaration is currently advisory only — nothing in the summary-refresh or sync paths branches on it.

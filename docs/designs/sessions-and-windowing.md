...
The declaration is currently advisory only — nothing in the summary-refresh or sync paths branches on it. Making the transcripts actually parseable (antigravity's SQLite store is readable read-only; gemini's checkpoints are cwd-scoped) is separate work.
